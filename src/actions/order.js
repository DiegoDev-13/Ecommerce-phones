...
export const createOrder = async (order) => {
    // 1 obtener el usuario autenticado + Client de la tabla customer
    ...
    for (const item of order.cartItems) {
        ...
        if(variantData.stock < item.quantity) {
            throw new Error('No hay suficiente stock para los articulos selecionados')
        }
    }

    // 3 Guardar la direccion de envio
    ...
    const {erorr: orderItemsError} = await supabase.from('order_items').insert(orderItems)
    ...
    for (const item of order.cartItems) {
        ...
        // Actulizar el stock de la variante 
...
export const getOrdersByCustomerId = async () => {
    ...
    const {data: customer, error: customerError} = await supabase.from('customers').select('id').eq('user_id', userId).single()
    ...
    const {data: orders, error: ordersError } = await supabase.from('orders').select('id, total_amount, status, created_at').eq('customer_id', customerId).order('created_at', {ascending: false})
